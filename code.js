...
function getFontStyleCandidates(targetStyle) {
    var _a;
    ...
    const rawCandidates = (_a = aliasMap[normalized]) !== null && _a !== void 0 ? _a : [targetStyle, "Regular", "Medium", "SemiBold", "Bold"];
    const seen = new Set();
    const candidates = [];
...
async function loadFontSafe(family, targetStyle) {
...
async function loadFontWithFallback(family, targetStyle) {
    const fonts = await figma.listAvailableFontsAsync();
    ...
    for (const fam of familyFallbacks) {
        ...
        for (const candidate of styleCandidates) {
            ...
            const usedDifferentStyle = normalizeFontStyleKey(match.fontName.style) !== requestedStyleKey;
            if (usedDifferentFamily || usedDifferentStyle) {
                sendLog(`⚠️ Font fallback used: requested ${family} ${targetStyle} → using ${match.fontName.family} ${match.fontName.style}`);
            }
            return match.fontName;
        }
        const regularish = familyFonts.find((f) => {
            return ["regular", "book", "normal"].includes(normalizeFontStyleKey(f.fontName.style));
        });
        if (regularish) {
            await figma.loadFontAsync(regularish.fontName);
            sendLog(`⚠️ Font fallback used: requested ${family} ${targetStyle} → using ${regularish.fontName.family} ${regularish.fontName.style}`);
            return regularish.fontName;
        }
        await figma.loadFontAsync(familyFonts[0].fontName);
        sendLog(`⚠️ Font fallback used: requested ${family} ${targetStyle} → using ${familyFonts[0].fontName.family} ${familyFonts[0].fontName.style}`);
        return familyFonts[0].fontName;
    }
    throw new Error(`No usable font found for ${family} ${targetStyle}`);
}
function removeExistingStyleByName(name) {
...
async function getVariableCollectionByName(name) {
...
function getVariablesInCollection(collection) {
...
function ensureMode(collection, modeName) {
    const existing = collection.modes.find((mode) => mode.name.toLowerCase() === modeName.toLowerCase());
    if (existing)
        return existing.modeId;
    return collection.addMode(modeName);
}
function ensureBaseModes(collection) {
    const findMode = (name) => collection.modes.find((m) => m.name.toLowerCase() === name.toLowerCase());
    ...
    if (!refreshedMobile)
        throw new Error("Failed to ensure mobile mode");
    if (!refreshedTablet)
        throw new Error("Failed to ensure tablet mode");
    if (!refreshedDesktop)
        throw new Error("Failed to ensure desktop mode");
}
function ensureNumberVariable(collection, variableName) {
    const existing = collection.variableIds
        .map((id) => figma.variables.getVariableById(id))
        .find((v) => v !== null && v.name === variableName);
    if (existing)
        return existing;
    return figma.variables.createVariable(variableName, collection, "FLOAT");
}
async function ensureTypographyStructure() {
...
function findVariable(variables, variableName) {
    var _a;
    return (_a = variables.find((v) => v.name === variableName)) !== null && _a !== void 0 ? _a : null;
}
function bindStyleToVariables(style, group, variables) {
    ...
    const missing = [];
    if (!fontSizeVar)
        missing.push(`${group}/font-size`);
    if (!lineHeightVar)
        missing.push(`${group}/line-height`);
    if (!letterSpacingVar)
        missing.push(`${group}/letter-spacing`);
    if (missing.length > 0) {
...
function snapTo4(value) {
    return Math.ceil(value / 4) * 4;
}
function calcLineHeight(size, type) {
    let raw = 0;
    if (type === "heading") {
        raw = size * 1.2;
    }
...
function calcLetterSpacing(size, type) {
    let em = 0;
    if (type === "h1")
        em = -0.025;
    else if (type === "h2")
...
function makeStyleDefs(base, ratio, fontFamily, selectedHeadingWeights, selectedBodyWeights) {
    const pow = (exp) => Math.pow(ratio, exp);
    ...
    const enabledHeadingWeights = HEADING_WEIGHTS.filter((weight) => selectedHeadingWeights.includes(weight.name));
    const enabledBodyWeights = BODY_WEIGHTS.filter((weight) => selectedBodyWeights.includes(weight.name));
    for (const spec of headingSpecs) {
        for (const weight of enabledHeadingWeights) {
            defs.push({
                ...
                letterSpacing: calcLetterSpacing(spec.size, spec.key),
            });
        }
    }
    const bodySpecs = [
    ...
    defs.sort((a, b) => {
        ...
        const baseDiff = styleOrder.indexOf(aBase) - styleOrder.indexOf(bBase);
        if (baseDiff !== 0)
            return baseDiff;
...
function buildStructuredTokenExport(mobileDefs, tabletDefs, desktopDefs) {
    const result = {
        meta: {
    ...
    const groups = Array.from(new Set(desktopDefs.map((def) => def.tokenGroup)));
    for (const group of groups) {
        ...
        if (!mobileDef || !tabletDef || !desktopDef)
            continue;
...
function buildStructuredStyleExport(defs) {
    const result = {
        meta: {
            generator: "TypeFlow v2",
            kind: "styles",
...
function buildCombinedExport(mobileDefs, tabletDefs, desktopDefs) {
    return {
        meta: {
            generator: "TypeFlow v2",
        ...
        tokens: buildStructuredTokenExport(mobileDefs, tabletDefs, desktopDefs),
        styles: buildStructuredStyleExport(desktopDefs),
    };
}
function buildCssExport(defs) {
    const lines = [":root {"];
    for (const def of defs) {
...
function isValidModeName(value) {
    return value === "mobile" || value === "tablet" || value === "desktop";
}
function validateImportedTypeFlowJson(data) {
...
function setImportedValuesForModes(imported, collection, variables) {
...
async function createStylesFromImportedJson(imported, variables) {
    const errors = [];
    let createdCount = 0;
    ...
    for (const [styleName, styleMeta] of Object.entries(importedStyles)) {
        try {
            ...
            const loadedFont = await loadFontWithFallback(styleMeta.fontFamily, styleMeta.fontStyle);
...
function sendCopyPayload(text) {
    figma.ui.postMessage({
        type: "copy-text",
        text,
    });
}
function getModeType(modeName) {
...
function getActiveModeIdForNode(node, collection) {
    var _a;
    return (_a = node.resolvedVariableModes[collection.id]) !== null && _a !== void 0 ? _a : collection.defaultModeId;
}
function getVariableValue(variables, name, modeId) {
...
function makeModePreviewLines(label, defs) {
    return [
        `${label}:`,
        ...defs.map((def) => {
            return `${def.name} → size ${def.fontSize}, line ${def.lineHeight}, tracking ${def.letterSpacing}`;
...
function findChildByRole(parent, role) {
    return parent.children.find((node) => node.getPluginData("typeflow-role") === role);
}
function setRole(node, role) {
    node.setPluginData("typeflow-role", role);
}
function getOrCreateSection(frame, role, name) {
...
function reorderPreviewSections(frame, defs) {
    const title = findChildByRole(frame, "preview-title");
    const subtitle = findChildByRole(frame, "preview-subtitle");
    // 제목과 서브타이틀 먼저 앞으로
    if (title && frame.children.indexOf(title) !== 0) {
        frame.insertChild(0, title);
    }
    if (subtitle) {
        const subtitleIndex = frame.children.indexOf(subtitle);
        if (subtitleIndex !== 1) {
            frame.insertChild(Math.min(1, frame.children.length), subtitle);
        }
    }
...
function getPreviewCategory(def) {
    if (def.tokenGroup.startsWith("h"))
        return "heading";
    if (def.tokenGroup === "caption")
        return "caption";
    return "body";
}
function getPreviewCategoryLabel(category) {
    if (category === "heading")
        return "Heading";
    if (category === "body")
        return "Body";
...
function reorderGroupedPreviewSections(frame) {
...
function makeModeMetaLine(label, fontSize, lineHeight, letterSpacing) {
    return `${label} ${formatPx(fontSize)} / ${formatPx(lineHeight)} / ${formatPx(letterSpacing)}`;
}
async function getOrCreateTextNode(parent, role, name, fallbackFont) {
...
async function getOrCreatePreviewFrame() {
...
function updateVariablesForAllModes(mobileDefs, tabletDefs, desktopDefs, collection, variables) {
    for (const mode of collection.modes) {
        const modeType = getModeType(mode.name);
        let defs;
        if (modeType === "mobile")
            defs = mobileDefs;
        else if (modeType === "tablet")
            defs = tabletDefs;
        else
            defs = desktopDefs;
        for (const def of defs) {
            const fontSizeVar = findVariable(variables, `${def.tokenGroup}/font-size`);
            const lineHeightVar = findVariable(variables, `${def.tokenGroup}/line-height`);
            const letterSpacingVar = findVariable(variables, `${def.tokenGroup}/letter-spacing`);
            if (!fontSizeVar || !lineHeightVar || !letterSpacingVar) {
...
async function applyTokens(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights) {
    const mobileDefs = makeStyleDefs(mobileBaseFontSize, mobileScaleRatio, fontFamily, headingWeights, bodyWeights);
    const tabletDefs = makeStyleDefs(tabletBaseFontSize, tabletScaleRatio, fontFamily, headingWeights, bodyWeights);
    const desktopDefs = makeStyleDefs(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
...
async function applyStyles(baseFontSize, scaleRatio, fontFamily, headingWeights, bodyWeights) {
    const defs = makeStyleDefs(baseFontSize, scaleRatio, fontFamily, headingWeights, bodyWeights);
...
async function applyBoth(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights) {
    const mobileDefs = makeStyleDefs(mobileBaseFontSize, mobileScaleRatio, fontFamily, headingWeights, bodyWeights);
    const tabletDefs = makeStyleDefs(tabletBaseFontSize, tabletScaleRatio, fontFamily, headingWeights, bodyWeights);
    const desktopDefs = makeStyleDefs(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
    const { collection, variables } = await ensureTypographyStructure();
...
async function previewTypographyFrame(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights) {
    const mobileDefs = makeStyleDefs(mobileBaseFontSize, mobileScaleRatio, fontFamily, headingWeights, bodyWeights);
    const tabletDefs = makeStyleDefs(tabletBaseFontSize, tabletScaleRatio, fontFamily, headingWeights, bodyWeights);
    const desktopDefs = makeStyleDefs(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
    ...
    const title = await getOrCreateTextNode(frame, "preview-title", "Preview Title", titleFont);
    ...
    const subtitle = await getOrCreateTextNode(frame, "preview-subtitle", "Preview Subtitle", bodyFont);
    subtitle.characters =
    ...
    const headingGroup = getOrCreateSection(frame, "preview-group-heading", "Heading Preview Group");
    const bodyGroup = getOrCreateSection(frame, "preview-group-body", "Body Preview Group");
    const captionGroup = getOrCreateSection(frame, "preview-group-caption", "Caption Preview Group");
    const groupSections = {
        heading: headingGroup,
        body: bodyGroup,
        caption: captionGroup,
    };
    for (const category of ["heading", "body", "caption"]) {
        ...
        const groupLabel = await getOrCreateTextNode(groupFrame, `preview-group-label-${category}`, `${category} Group Label`, titleFont);
    ...
    for (const def of defs) {
        ...
        const groupFrame = groupSections[category];
        const itemSection = getOrCreateSection(groupFrame, `preview-item-${def.group}`, `${def.name} Preview Item`);
        ...
        const nameText = await getOrCreateTextNode(itemSection, `preview-name-${def.group}`, `${def.name} Name`, bodyFont);
        ...
        const desktopLetterSpacing = desktopMode
            ...
            : null;
        const metaText = await getOrCreateTextNode(itemSection, `preview-meta-${def.group}`, `${def.name} Meta`, bodyFont);
        metaText.characters = [
...
async function previewTypography(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights) {
    await previewTypographyFrame(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
}
async function exportJson(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights) {
    const mobileDefs = makeStyleDefs(mobileBaseFontSize, mobileScaleRatio, fontFamily, headingWeights, bodyWeights);
    const tabletDefs = makeStyleDefs(tabletBaseFontSize, tabletScaleRatio, fontFamily, headingWeights, bodyWeights);
    const desktopDefs = makeStyleDefs(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
    const json = JSON.stringify(buildCombinedExport(mobileDefs, tabletDefs, desktopDefs), null, 2);
    sendLog(json);
    figma.notify("Structured JSON exported 🎉");
}
async function exportCss(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights) {
    const defs = makeStyleDefs(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
    const css = buildCssExport(defs);
    sendLog(css);
    figma.notify("CSS exported");
}
async function copyJson(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights) {
    const mobileDefs = makeStyleDefs(mobileBaseFontSize, mobileScaleRatio, fontFamily, headingWeights, bodyWeights);
    const tabletDefs = makeStyleDefs(tabletBaseFontSize, tabletScaleRatio, fontFamily, headingWeights, bodyWeights);
    const desktopDefs = makeStyleDefs(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
    const json = JSON.stringify(buildCombinedExport(mobileDefs, tabletDefs, desktopDefs), null, 2);
...
async function copyCss(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights) {
    const defs = makeStyleDefs(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
    const css = buildCssExport(defs);
    sendCopyPayload(css);
    sendLog(css);
...
async function importTypeFlowJson(importJsonText) {
    let parsed;
    try {
    ...
    const { createdCount, errors } = await createStylesFromImportedJson(imported, variables);
    if (errors.length > 0) {
        sendLog([
...
async function sendPresetListToUI(selectedName) {
    const presets = await getStoredPresets();
    figma.ui.postMessage({
        type: "preset-list",
        presets,
        selectedName: selectedName !== null && selectedName !== void 0 ? selectedName : "",
    });
}
figma.ui.onmessage = async (msg) => {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m;
    try {
        const fontFamily = (_a = msg.fontFamily) !== null && _a !== void 0 ? _a : "Inter";
        const headingWeights = (_b = msg.headingWeights) !== null && _b !== void 0 ? _b : ["Regular", "Medium", "SemiBold"];
        const bodyWeights = (_c = msg.bodyWeights) !== null && _c !== void 0 ? _c : ["Regular", "Medium"];
        const mobileBaseFontSize = (_d = msg.mobileBaseFontSize) !== null && _d !== void 0 ? _d : 16;
        const mobileScaleRatio = (_e = msg.mobileScaleRatio) !== null && _e !== void 0 ? _e : 1.2;
        const tabletBaseFontSize = (_f = msg.tabletBaseFontSize) !== null && _f !== void 0 ? _f : 17;
        const tabletScaleRatio = (_g = msg.tabletScaleRatio) !== null && _g !== void 0 ? _g : 1.22;
        const desktopBaseFontSize = (_h = msg.desktopBaseFontSize) !== null && _h !== void 0 ? _h : 18;
        const desktopScaleRatio = (_j = msg.desktopScaleRatio) !== null && _j !== void 0 ? _j : 1.25;
        if (msg.type === "test") {
            debugCollections();
        ...
        if (msg.type === "save-preset") {
            const name = (_k = msg.presetName) === null || _k === void 0 ? void 0 : _k.trim();
        ...
        if (msg.type === "delete-preset") {
            const name = (_l = msg.presetName) === null || _l === void 0 ? void 0 : _l.trim();
            if (!name) {
        ...
        if (msg.type === "preview") {
            await applyBoth(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
            await previewTypography(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
            return;
        }
        ...
        if (msg.type === "export-json") {
            await exportJson(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
            return;
        }
        if (msg.type === "export-css") {
            await exportCss(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
            return;
        }
        if (msg.type === "copy-json") {
            await copyJson(mobileBaseFontSize, mobileScaleRatio, tabletBaseFontSize, tabletScaleRatio, desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
            return;
        }
        if (msg.type === "copy-css") {
            await copyCss(desktopBaseFontSize, desktopScaleRatio, fontFamily, headingWeights, bodyWeights);
            return;
        }
        if (msg.type === "import-json") {
            const importJsonText = (_m = msg.importJsonText) === null || _m === void 0 ? void 0 : _m.trim();
            if (!importJsonText) {
